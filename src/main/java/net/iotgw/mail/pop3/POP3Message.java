...
public class POP3Message extends MimeMessage implements ReadableMime {
			...
			synchronized (this) {
				...
				if (rawcontent == null) {
					...
					if (headers == null || ((POP3Store) (folder.getStore())).forgetTopHeaders) {
						...
						/*
						 * Already have the headers, have to skip the headers in the content array and
						 * return the body.
						 *
						 * XXX - It seems that some mail servers return slightly different headers in
						 * the RETR results than were returned in the TOP results, so we can't depend on
						 * remembering the size of the headers from the TOP command and just skipping
						 * that many bytes. Instead, we have to process the content, skipping over the
						 * header until we come to the empty line that separates the header from the
						 * body.
						 */
						int offset = 0;
						for (;;) {
	...
	public Date getReceivedDate() throws MessagingException {
		...
		if (headers == null || !headers.getAllHeaders().hasMoreElements()) {
			//System.out.println(POP3Message.class.getName()+".getReceivedDate() - headers is null");
			loadHeaders();
		}
		//System.out.println(POP3Message.class.getName()+".getReceivedDate() - has more emel:"+headers.getAllHeaders().hasMoreElements());
		...
		//System.out.println(POP3Message.class.getName()+".getReceivedDate() - "+recvDateText);
	...
	private void loadHeaders() throws MessagingException {
		...
		try {
			...
			synchronized (this) {
				if (headers != null && headers.getAllHeaderLines().hasMoreElements()) // check again under lock
				{
					System.out.println(POP3Message.class.getName()+".loadHeaders() - has more emel:"+headers.getAllHeaders().hasMoreElements());
					return;
				}
				...
				if (((POP3Store) (folder.getStore())).disableTop
					...
					// content.
					System.out.println(POP3Message.class.getName()+".loadHeaders() - top is disabled.");
					fetchContent = true;
				} else {
					try {
